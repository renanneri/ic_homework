...
#         best_index = df[ ['param_{}'.format(param), 'rank_test_%s' % scorer] ].sort_values(by='param_{}'.format(param), ascending=False).loc[0]["param_{}".format(param)]
        
#         worst_index = df['rank_test_%s' % scorer].sort_values(ascending=False).index[-1]
#         best_score = df['mean_test_%s' % scorer][best_index]
#         worst_score = df['mean_test_%s' % scorer][worst_index]
